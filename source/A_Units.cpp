...
#define UGLY_CODE if(angle == 0x80000000) newLabel += "max" + aste;\
    ...
    Units_LanguageDLLConverter[0]->SetLabel(boost::lexical_cast<std::string>(Units_DLL_LanguageHelp->index));
    Units_DLL_LanguageHKText->SetLabel(TranslatedText(Units_DLL_LanguageHKText->index, 64));
    Units_LanguageDLLConverter[1]->SetLabel(boost::lexical_cast<std::string>(Units_DLL_LanguageHKText->index));
    ...
    if (selections > 0 && GenieVersion >= genie::GV_AoK && (GenieVersion < genie::GV_C15 || GenieVersion > genie::GV_LatestDE2))
    {
        ...
        Units_UnitHeads_Name->SetLabel(" " + boost::lexical_cast<std::string>(UnitIDs.front()) + " - " + GetUnitName(UnitIDs.front(), 0));
    }
    else    // AoE or RoR
    {
...
{
    return boost::lexical_cast<std::string>((short)dataset->Civs[UnitCivID].Units[UnitIDs.front()].DamageGraphics[index].DamagePercent)
        + " % - ID: " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].DamageGraphics[index].GraphicID);
}
    ...
    if (selections > 0)
    {
        ...
        for (size_t sel = selections; sel-- > 0;)
        {
            for (short vecCiv = (CopyGraphics) ? SelectedCivs.size() : 1; vecCiv-- > 0;)
            {
                ...
                {
                    warning.Append(boost::lexical_cast<std::string>(SelectedCivs[vecCiv]) + " ");
                    showWarning = true;
                }
        ...
        if (showWarning)
        {
            warning.Append("\ndiffers from civ " + boost::lexical_cast<std::string>(UnitCivID));
...
{
    return "Amount: " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].Type50.Attacks[index].Amount)
        + " - Class " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].Type50.Attacks[index].Class);
}
...
{
    return "Amount: " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].Type50.Armours[index].Amount)
        + " - Class " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].Type50.Armours[index].Class);
}
...
wxString AGE_Frame::GetUnitCommandName(int index)
{
    ...
    {
        return "Unk. Type " + boost::lexical_cast<std::string>(CommandType);
    }
}
        ...
        if (Units_ListV->GetSelectedCount())
            for (size_t loop = 0; loop < dataset->UnitHeaders[UnitIDs.front()].TaskList.size(); ++loop)
            {
                ...
                if (dataset->UnitHeaders[UnitIDs.front()].TaskList[loop].ClassID != -1)
                    Name += " class " + boost::lexical_cast<std::string>(dataset->UnitHeaders[UnitIDs.front()].TaskList[loop].ClassID);
                else if (dataset->UnitHeaders[UnitIDs.front()].TaskList[loop].UnitID != -1)
                    Name += " unit " + boost::lexical_cast<std::string>(dataset->UnitHeaders[UnitIDs.front()].TaskList[loop].UnitID);
                if (SearchMatches(" " + Name.Lower() + " "))
                {
            ...
            && dataset->Civs[UnitCivID].Units[UnitIDs.front()].Type <= 80)
        {
            ...
            for (size_t loop = 0; loop < dataset->Civs[UnitCivID].Units[UnitIDs.front()].Bird.TaskList.size(); ++loop)
            {
                ...
                if (dataset->Civs[UnitCivID].Units[UnitIDs.front()].Bird.TaskList[loop].ClassID != -1)
                    Name += " class " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].Bird.TaskList[loop].ClassID);
                else if (dataset->Civs[UnitCivID].Units[UnitIDs.front()].Bird.TaskList[loop].UnitID != -1)
                    Name += " unit " + boost::lexical_cast<std::string>(dataset->Civs[UnitCivID].Units[UnitIDs.front()].Bird.TaskList[loop].UnitID);
                if (SearchMatches(" " + Name.Lower() + " "))
                {
                    Units_Tasks_ListV->names.Add(Name);
                    Units_Tasks_ListV->indexes.push_back(loop);
...
void AGE_Frame::UnitLangDLLConverter(wxCommandEvent& event)
{
    ...
    try
    {
        DLLValue = boost::lexical_cast<int32_t>(static_cast<wxTextCtrl*>(event.GetEventObject())->GetValue());
    }
        ...
        Units_Trait_CheckBox[loop]->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent& event)
            {
                ...
                try
                {
                    uint8_t attribute = boost::lexical_cast<short>(Units_Trait->GetValue());
        ...
        Units_GarrisonType_CheckBox[loop]->Bind(wxEVT_CHECKBOX, [this](wxCommandEvent& event)
            {
                ...
                try
                {
                    uint8_t type = boost::lexical_cast<short>(Units_GarrisonType->GetValue());
