...
{
    ChangeValue(boost::lexical_cast<std::string>(value));
}
...
int NumberControl::SaveChangesAsUByte(bool forced)
{
    ...
    if (value.size() > 0)
    {
        ...
        try
        {
            uint8_t casted = (uint8_t)boost::lexical_cast<short>(value);
            if (boost::lexical_cast<short>(value) == casted)
            {
                if (batchMode > 0)
                {
                    ...
                    ChangeValue(boost::lexical_cast<std::string>((short)*(uint8_t*)container.back()));
            ...
            {
                boost::throw_exception(boost::bad_lexical_cast(typeid(short), typeid(std::string)));
            }
        }
...
int NumberControl::SaveChangesAsFloat(bool forced)
{
    ...
    if (value.size() > 0)
    {
        ...
        try
        {
            ...
            else casted = boost::lexical_cast<float>(value);
            if (batchMode > 0)
            {
                for (void* pointer : container)
                {
                ...
                ChangeValue(boost::lexical_cast<std::string>(*(float*)container.back()));
...
int NumberControl::SaveChangesAsLong(bool forced)
{
    ...
    if (value.size() > 0)
    {
        ...
        try
        {
            int32_t casted = boost::lexical_cast<int32_t>(value);
            if (batchMode > 0)
            {
                for (void* pointer : container)
                {
                ...
                ChangeValue(boost::lexical_cast<std::string>(*(int32_t*)container.back()));
...
int NumberControl::SaveChangesAsULong(bool forced)
{
    ...
    if (value.size() > 0)
    {
        ...
        try
        {
            uint32_t casted = boost::lexical_cast<uint32_t>(value);
            if (batchMode > 0)
            {
                for (void* pointer : container)
                {
                ...
                ChangeValue(boost::lexical_cast<std::string>(*(uint32_t*)container.back()));
...
int NumberControl::SaveChangesAsShort(bool forced)
{
    ...
    if (value.size() > 0)
    {
        ...
        try
        {
            int16_t casted = boost::lexical_cast<int16_t>(value);
            if (batchMode > 0)
            {
                for (void* pointer : container)
                {
                ...
                ChangeValue(boost::lexical_cast<std::string>(*(int16_t*)container.back()));
    ...
    if (*(std::string*)container.back() != value || forced) // Has been changed
    {
        if (value.length() >= 1 && (char)value[0] == '/')
        {
            ...
            case '-':
            {
                ...
                try
                {
                    cut = boost::lexical_cast<int>(value.substr(2));
                }
                catch (const boost::bad_lexical_cast&)
                {
                    frame->popUp.post("Please enter a number from 0 to " + boost::lexical_cast<std::string>(maxSize), BWTITLE, this);
                    return 2;
                }
                for (void* pointer : container)
                {
