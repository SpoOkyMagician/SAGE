...
void AGE_Frame::OnOpen(wxCommandEvent&)
{
    if (popUp.unSaved > 0)
    {
        int answer = wxMessageBox("Do you want to save changes made to open files?\nThere are "
            + boost::lexical_cast<std::string>(popUp.unSaved) + " unsaved changes.",
    ...
    if (!SkipOpenDialog)
    {
        ...
        int RecentItems, RecentVersion;
        {
            ...
            for (int i = 0; i < RecentItems; ++i)
            {
                ...
                if (RecentVersion < 2 && boost::lexical_cast<int>(temp) > EV_TC)
                {
                    temp = "-1";
                }
                if (RecentVersion < 4)
                {
                    int version = boost::lexical_cast<int>(temp);
                    if (version > EV_EF)
                    {
                        ++version;
                        temp = boost::lexical_cast<std::string>(version);
                    }
                }
        ...
        OpenBox.TerrainsBox->ChangeValue(boost::lexical_cast<std::string>(CustomTerrains));
        ...
        CustomTerrains = boost::lexical_cast<int>(OpenBox.TerrainsBox->GetValue());
        ...
        {
            wxConfig Config("", "", "AGE2\\ConfigWindow" + boost::lexical_cast<std::string>(window_num + 1), "", wxCONFIG_USE_LOCAL_FILE | wxCONFIG_USE_RELATIVE_PATH);
        ...
        latest.Alloc(13);
        latest.Add(boost::lexical_cast<std::string>(GameVersion));
    ...
    {   // Without these, nothing can be edited.
        ...
        SetStatusText(boost::lexical_cast<std::string>(dataset->FileVersion), 4);
        DataOpened = true;
        TabBar_Main->Freeze();
        //OnGameVersionChange();
...
void AGE_Frame::OnSave(wxCommandEvent&)
{
...

    {
        wxArrayString latest;
        latest.Alloc(5);
        latest.Add(boost::lexical_cast<std::string>(SaveGameVersion));
    ...
    if (SaveDat)
    {
        ...
        if (GenieVersion != genie::GV_TCV && GenieVersion != genie::GV_CCV && GenieVersion != genie::GV_CCV2)
        {
            ...
            {
                wxString viesti = "Send file to Tapsa for repair!\nTerrains: " + boost::lexical_cast<std::string>(TerrainsInData);
                viesti += "\nBorders: " + boost::lexical_cast<std::string>(BordersInTerrains);
                viesti += "\nLoaded game version: " + boost::lexical_cast<std::string>(dataset->TerrainBlock.getGameVersion());
                viesti += "\nTerrain game version: " + boost::lexical_cast<std::string>(dataset->TerrainBlock.Terrains.front().getGameVersion());
                wxMessageBox(viesti);
            }
            // <-- ends here
        }
    ...
    SetStatusText("Selected files saved. " + boost::lexical_cast<std::string>(popUp.unSaved) + " dat edits.", 0);
    popUp.unSaved = 0;
}
...
void AGE_Frame::OnMenuOption(wxCommandEvent& event)
{
    ...
    case eAddWindow:
    {
        for (size_t win = 0; win < 4; ++win)
            if (!openEditors[win])
            {
                AGE_Frame* newWindow = new AGE_Frame("AGE " + AboutDialog::AGE_VER + " window " + boost::lexical_cast<std::string>(win + 1), win);
                wxCommandEvent open;
    ...
    case eCompileList:
    {
        ...
        for (size_t sprite = 0; sprite < dataset->Graphics.size(); ++sprite)
            ...
            {
                slp_csv.AddLine(boost::lexical_cast<std::string>(dataset->Graphics[sprite].SLP)
                    + ',' + dataset->Graphics[sprite].FileName
                    + ',' + dataset->Graphics[sprite].Name
                    + ',' + boost::lexical_cast<std::string>(dataset->Graphics[sprite].FrameCount)
                    + ',' + boost::lexical_cast<std::string>(dataset->Graphics[sprite].AngleCount)
                    + ',' + boost::lexical_cast<std::string>(dataset->Graphics[sprite].AnimationDuration)
                    + ',' + boost::lexical_cast<std::string>(dataset->Graphics[sprite].ReplayDelay));
            }
                ...
                if (sprite < dataset->Graphics.size() && dataset->GraphicPointers[sprite])
                {
                    if (!dataset->Graphics[sprite].FileName.empty())
                    {
                        obj_csv.AddLine(dataset->Civs[civ].Name
                            + ',' + boost::lexical_cast<std::string>((int)dataset->Civs[civ].IconSet)
                            + ',' + boost::lexical_cast<std::string>(unit)
                            ...
                            + ',' + dataset->Graphics[sprite].Name
                            + ',' + dataset->Graphics[sprite].FileName
                            + ',' + boost::lexical_cast<std::string>((int)dataset->Graphics[sprite].Layer)
                            + ',' + boost::lexical_cast<std::string>(dataset->Graphics[sprite].FrameCount)
                            + ',' + boost::lexical_cast<std::string>(dataset->Graphics[sprite].AngleCount));
                    }
                    if (depth == 1)
    ...
    case eBoxWidth:
    {
        ...
        if (ted.ShowModal() == wxID_OK)
        {
            try
            {
                boxWidthMultiplier = boost::lexical_cast<float>(ted.GetValue());
    ...
    case eScrollRate:
    {
        ...
        if (ted.ShowModal() == wxID_OK)
        {
            try
            {
                AScrolled::rate = boost::lexical_cast<float>(ted.GetValue()) / 120.f;
            }
            catch (const boost::bad_lexical_cast&)
            {
                wxMessageBox("Bad floating point", "AGE");
    ...
    default: wxMessageBox("ID " + boost::lexical_cast<std::string>(event.GetId()) + "\nType " + boost::lexical_cast<std::string>(event.GetEventType()), "Unhandled Event");
    }
}
...
void AGE_Frame::LoadTXT(const wxString& filename)
{
    ...
    while (getline(infile, line))
    {
        ...
        if (num)
        {
            size_t ID = boost::lexical_cast<size_t>(line.substr(0, num));
...
void AGE_Frame::OnKillFocus_LangDLL(wxFocusEvent& event)
{
    ...
    if (control->IsModified())
    {
        ...
        SetStatusText("Wrote \"" + Name + "\" to " + boost::lexical_cast<std::string>(ID), 0);
    }
}
