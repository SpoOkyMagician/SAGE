        ...
        auto Populate89 = [this](float data)
            {
                ...
                Effects_89_Amount->ChangeValue(boost::lexical_cast<std::string>(amount)); // Correct value
                Effects_89_Type->Show(true);
                Effects_89_Type_CB1->Show(true);
                Effects_89_Type->ChangeValue(boost::lexical_cast<std::string>(type)); // Correct class
...
void AGE_Frame::LoadAllEffects(wxCommandEvent& event)
{
    ...
    for (short tech = 0; tech < dataset->Effects.size(); ++tech)
    {
        ...
        {
            wxString Name = " T" + boost::lexical_cast<std::string>(tech) +
                " E" + boost::lexical_cast<std::string>(effect) + " - " + GetEffectCmdName(effect, tech);
            if (SearchMatches(" " + Name.Lower() + " "))
            {
    ...
    Effects_89_Type_CB1->Bind(wxEVT_COMBOBOX, [this](wxCommandEvent& event)
        {
            ...
            try
            {
                amount = boost::lexical_cast<int16_t>(Effects_89_Amount->GetValue());
            }
            ...
            std::string newValue(boost::lexical_cast<std::string>(CombineEffects89(type, amount)));
...
void AGE_Frame::UpdateEffects89(bool forced)
{
    ...
    if (!Effects_89_Amount->IsEmpty() && !Effects_89_Type->IsEmpty())
    {
        ...
        try
        {
            type = boost::lexical_cast<uint16_t>(Effects_89_Type->GetValue());
        }
        catch (const boost::bad_lexical_cast&)
        {
