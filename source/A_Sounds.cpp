...
void AGE_Frame::OnSoundItemSelect(wxCommandEvent& event)
{
    if (!SoundIDs.empty())
    {
        ...
        Sounds_TotalProbability_Info->SetLabel("Used " + boost::lexical_cast<std::string>(sum) + "/" + boost::lexical_cast<std::string>(dataset->Sounds[SoundIDs.front()].TotalProbability));
    }
...
void AGE_Frame::LoadAllSoundFiles(wxCommandEvent& event)
{
    ...
    for (short sound = 0; sound < dataset->Sounds.size(); ++sound)
    {
        ...
        {
            wxString Name = " S" + boost::lexical_cast<std::string>(sound) + " F" + boost::lexical_cast<std::string>(file) + " - " + GetSoundItemName(file, sound);
            if (SearchMatches(" " + Name.Lower() + " "))
            {
