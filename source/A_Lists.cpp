...
void AGE_Frame::FillListsBasedOnGameVersion()
{
    ...
    if (Customs.Read(KeyNumArmors, &NumNames, 0))
    {
        ...
        {
            wxString indexAsText = boost::lexical_cast<std::string>(loop);
    ...
    else
    {
        ...
        public:
            ...
            {
                wxString indexAsText = boost::lexical_cast<std::string>(index++);
    ...
    public:
        ...
        void Add(wxString name)
        {
            wxString line = key + boost::lexical_cast<std::string>(index++);
    ...
    if (Customs.Read(KeyNumTerrainTables, &NumNames, 0))
    {
        for (size_t loop = 0; loop < NumNames; ++loop)
        {
            wxString line = KeyTerrainTableNames + boost::lexical_cast<std::string>(loop);
    ...
    if (Customs.Read(KeyNumCivResources, &NumNames, 0))
    {
        for (size_t loop = 0; loop < NumNames; ++loop)
        {
            wxString line = KeyCivResourceNames + boost::lexical_cast<std::string>(loop);
            Customs.Read(line, &read_buf, "Unused");
...
void AGE_Frame::SearchAllSubVectors(ProperList* list, wxTextCtrl* topSearch, wxTextCtrl* subSearch)
{
    ...
    for (size_t loop = 0; loop < selections; ++loop)
    {
        ...
        topNums.insert(boost::lexical_cast<uint32_t>(line.substr(2, found - 2)));
        subNums.insert(boost::lexical_cast<uint32_t>(line.substr(2 + found, line.find(" ", found + 3) - found - 2)));
    }
    wxString topText;
    for (unsigned num : topNums)
    {
        topText += " " + boost::lexical_cast<std::string>(num) + " -|";
    }
    ...
    if (FilterAllSubs)
    {
        ...
        {
            subText += " " + boost::lexical_cast<std::string>(num) + " -|";
        }
...
void AGE_Frame::getSelectedItems(const size_t selections, const ProperList* list, std::vector<int>& indexes)
{
    ...
    SetStatusText("Times listed: " + boost::lexical_cast<std::string>(++times_listed), 2);
}
