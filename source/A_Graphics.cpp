...
void AGE_Frame::OnGraphicSelect(wxCommandEvent& event)
{
    ...
    if (GraphicPointer)
    {
        ...
        Graphics_MirroringMode->SetToolTip("If used, should be " +
            boost::lexical_cast<std::string>(goodMirrorMode) + " for this sprite.\n" + MirrorHelp);
...
wxString AGE_Frame::GetGraphicDeltaName(int index)
{
    ...
    if (deltaID < dataset->Graphics.size())
        return boost::lexical_cast<std::string>(deltaID) + ": " + GetGraphicName(deltaID, false);
    return "Re-drawer " + boost::lexical_cast<std::string>(deltaID);
}
                ...
                while (getline(infile, line))
                {
                    ...
                    for (size_t loop = 1, spriteID = static_cast<size_t>(-1), deltaID = static_cast<size_t>(-1); loop < pieces.GetCount(); ++loop)
                    {
                        ...
                        case 1: // ID
                        {
                            try
                            {
                                spriteID = boost::lexical_cast<uint32_t>(pieces[1]);
                            }
                        ...
                        case 2: // Either index of the delta or the sprite number of the delta.
                        {
                            try
                            {
                                deltaID = boost::lexical_cast<uint32_t>(pieces[2]);
                            }
                            catch (const boost::bad_lexical_cast&)
                            {
                        ...
                        case 4: // Offset X
                        {
                            ...
                            try
                            {
                                dataset->Graphics[spriteID].Deltas[deltaID].OffsetX = boost::lexical_cast<int16_t>(pieces[4]);
                            }
                            catch (const boost::bad_lexical_cast&)
                            {
                        ...
                        case 5: // Offset Y
                        {
                            ...
                            {
                                dataset->Graphics[spriteID].Deltas[deltaID].OffsetY = boost::lexical_cast<int16_t>(pieces[5]);
                            }
                            catch (const boost::bad_lexical_cast&)
                            {
