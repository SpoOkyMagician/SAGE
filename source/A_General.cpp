...
void AGE_Frame::OnVariableCalc(wxFocusEvent& event)
{
    ...
    if (!General_CalcBoxes[0]->IsEmpty())
    {
        try
        {
            result = boost::lexical_cast<int32_t>(General_CalcBoxes[0]->GetValue()) & 0xFF;
        }
    ...
    if (!General_CalcBoxes[1]->IsEmpty())
    {
        try
        {
            result += (boost::lexical_cast<int32_t>(General_CalcBoxes[1]->GetValue()) & 0xFF) << 8;
        }
        catch (const boost::bad_lexical_cast&) {}
    }
    ...
    if (!General_CalcBoxes[2]->IsEmpty())
    {
        try
        {
            result += (boost::lexical_cast<int32_t>(General_CalcBoxes[2]->GetValue()) & 0xFF) << 16;
        }
        catch (const boost::bad_lexical_cast&) {}
    }
    ...
    if (!General_CalcBoxes[3]->IsEmpty())
    {
        try
        {
            result += (boost::lexical_cast<int32_t>(General_CalcBoxes[3]->GetValue()) & 0xFF) << 24;
        }
        catch (const boost::bad_lexical_cast&) {}
    }

    General_CalcBoxes[4]->ChangeValue(boost::lexical_cast<std::string>(result));
}
...
void AGE_Frame::OnVariableCalcReverse(wxFocusEvent& event)
{
    ...
    try
    {
        result = boost::lexical_cast<int32_t>(General_CalcBoxes[4]->GetValue());
    }
    catch (const boost::bad_lexical_cast&)
    {
        result = 0;
    ...
    General_CalcBoxes[0]->ChangeValue(boost::lexical_cast<std::string>(result & 0xFF));
    result >>= 8;
    General_CalcBoxes[1]->ChangeValue(boost::lexical_cast<std::string>(result & 0xFF));
    result >>= 8;
    General_CalcBoxes[2]->ChangeValue(boost::lexical_cast<std::string>(result & 0xFF));
    result >>= 8;
    General_CalcBoxes[3]->ChangeValue(boost::lexical_cast<std::string>(result & 0xFF));
}
...
wxString AGE_Frame::GetRandomMapName(int index)
{
    if (GenieVersion >= genie::GV_AoK)
    {
        return "Map " + boost::lexical_cast<std::string>(dataset->RandomMaps.Maps[index].MapID);
    }
    wxString Name;
    switch (index)
    {
    ...
    return Name += boost::lexical_cast<std::string>(dataset->RandomMaps.Maps[index].MapID) + ")";
}
...
{
    return "Land " + boost::lexical_cast<std::string>(dataset->RandomMaps.Maps[RandomMapIDs.front()].MapLands[index].LandID);
}
