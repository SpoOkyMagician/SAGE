...
void AGE_Frame::OnAnimationMenuOption(wxCommandEvent& event)
{
    ...
    case eShowSLP:
    {
        ...
        if (ShowSLP)
        {
            ...
            slp_zoom_btn = new wxButton(panel, eZoom, "Zoom: " + boost::lexical_cast<std::string>(int(slp_zoom * 100)) + " %");
    ...
    case eCacheDepth:
    {
        wxTextEntryDialog ted(this, "Enter new cache size in megabytes",
            "Set Cache Size", boost::lexical_cast<std::string>(GG::cache_size / 1000000u));
        ted.SetTextValidator(wxFILTER_DIGITS);
        if (ted.ShowModal() == wxID_OK)
        {
            int temp = boost::lexical_cast<int>(ted.GetValue());
...
bool AGE_Frame::LoadSLP(AGE_SLP* graphic)
{
    ...
    if (LooseHD)
    {
        ...
        for (int i = 0; i < folders.size(); ++i)
        {
            ...
            graphic->slp = GG::LoadSLP(folders[i] + boost::lexical_cast<std::string>(graphic->slpID) + ".slp");
            if (graphic->slp) return true;
        }
    }
    else
...
void AGE_Frame::BitmapToSLP(AGE_SLP* graphic)
{
    ...
    if (!frame)
    {
        wxMessageBox("Congrats seeing this message", "No SLP frame " + boost::lexical_cast<std::string>(graphic->frameID));
        return;
    }
...
void AGE_Frame::OnFrameButton(wxCommandEvent& event)
{
    ...
    case eZoom:
    {
        wxTextEntryDialog ted(this, "Enter new zooming %", "Set Scale Factor", boost::lexical_cast<std::string>(int(slp_zoom * 100)));
        ted.SetTextValidator(wxFILTER_DIGITS);
        if (ted.ShowModal() == wxID_OK)
        {
            int zoom_percent = std::min(800, boost::lexical_cast<int>(ted.GetValue()));
            slp_zoom = zoom_percent / 100.f;
            slp_zoom_btn->SetLabel("Zoom: " + boost::lexical_cast<std::string>(zoom_percent) + " %");
        }
        break;
    }
